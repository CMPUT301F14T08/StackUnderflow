...
/**
 * ProfileActivity This is the view for the view profile attributes. This displays how many
 * questions and answers a user has submitted. This also displays all of a users read later,
 * favorites and personal posts. Finally this is also the main location where the user can view his
 * name and change it.
 * 
...
public class ProfileActivity extends Activity implements TabListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        TextView mUserName = (TextView) actionBar.getCustomView().findViewById(
                R.id.profile_user_name);
        TextView mUserLocation = (TextView) actionBar.getCustomView().findViewById(
                R.id.user_location_prof);
        TextView mQuestionsPosted = (TextView) actionBar.getCustomView().findViewById(
                R.id.profile_questions_box);
        TextView mAnswersPosted = (TextView) actionBar.getCustomView().findViewById(
                R.id.profile_answers_box);
        ...
        if (userLoc != null) {
            if ((userLoc.latitude != LocManager.LOC_ERROR)
                    && (userLoc.latitude != LocManager.LOC_ERROR)) {
        ...
        mRatingBar.setRating(q_count + a_count < 25 ? (int) Math.floor((q_count + a_count) / 5)
                : 5.0f);
        ...
        formattedString.setSpan(new RelativeSizeSpan(0.4f), q_string.length(),
        ...
        formattedString.setSpan(new RelativeSizeSpan(0.4f), a_string.length(),
                formattedString.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
